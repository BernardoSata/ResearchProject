...
class Node:
    ...
    def __str__(self):
        "parent",self.parent,"cost",self.cost,"node","probability",self.probability
...
class Env_f():
    ...
    def expand_and_update(self, nodes, timeout, map_size):
        #print('Expanding')
        ...
        for i in range(len_n):
            args.append((nodes[i].requests, nodes[i].drones, timeout))
        ...
        for i in range(len(actions)):
            self.add_children(nodes[i], actions[i][0], actions[i][1], actions[i][2], map_size)
        ...
        for node in nodes:
            ...
            for children_list in node.children_lists:
                ...
                for child in children_list:
                    prob.append(child[1])
                    values.append(child[0].value)
                action_scores[index] = np.sum(np.asarray(prob) * np.asarray(np.asarray(node.actions[index][2]) + np.asarray(values)))\
                 / np.sum(prob)
                index += 1
            """
            costs = []
...
class Env_GA:
    ...
    def select_elite(self, population, percentile):
        ...
        return elite, bottom, best, -np.max(scores)
        
    def new_generation(self, population, prob_mutation, prob_elite):
        elite, bottom, best, best_value = self.select_elite(population,60) # Top 40%
        ...
        return new_population, best_value
...
def plan_GA(input):
    ...
    while time() - start < timeout:
        population, best = env_GA.new_generation(population, prob_mutation , prob_elite)
        history.append(best)
        
    """
    while converged(history) == False and time() - start < timeout:
        population, best = env_GA.new_generation(population, prob_mutation , prob_elite)
        history.append(best)
...
def converged(history):
    if len(history) < 100:
        return False
    hist_mean = np.mean(history[-300:])
    if hist_mean < 1.00001*history[-1] and hist_mean > 0.99999*history[-1]:
        return True
    else:
        return False
...
def computeAllocation(request_buffer, drones_available, max_time, timeout, map_size):
    ...
    while time() - start < max_time:
        nodes = root.select_best_action()
        env_f.expand_and_update(nodes, timeout, map_size)
